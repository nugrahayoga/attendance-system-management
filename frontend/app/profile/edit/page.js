...
class EditProfile extends React.Component {
	...
	getUser() {
		...
		axios({
			url: constants.NEXT_BASE_API_URL + "/employee/get-employee",
			headers: {
	...
	handleSubmit(e) {
		...
		axios({
			url: constants.NEXT_BASE_API_URL + "/employee/update",
			headers: {
				Authorization: `Bearer ${this.state.token}`,
			},
			data: dataToSubmit,
			...
			.then((response) => {
				if (response.status == 200) {
					...
					toast.success("Data berhasil diubah", { id: loadingToast })
					this.setState({
						loading: false,
					})
				} else {
		...
		return (
			<Layout>
				<div className="mx-auto max-w-270">
					<Breadcrumb pageName="Edit Profil" />
